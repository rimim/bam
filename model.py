        ...
        if self.load_dependent:
            ...
            if self.stribeck:
                ...
                if self.quadratic:
                    ...
                    self.load_friction_motor_quad_stribeck = Parameter(0.0, 0.0, 0.01)
                    self.load_friction_external_quad_stribeck = Parameter(
                        0.0, 0.0, 0.01
                    )
            ...
            if self.load_dependent:
                ...
                if self.quadratic and np.sign(external_torque) != np.sign(motor_torque):
                    if abs(external_torque) < abs(motor_torque):
                        ...
                        frictionloss += (
                            stribeck_coeff
                            * self.load_friction_external_quad_stribeck.value
                            * abs(external_torque) ** 2
                        )
                    else:
                        frictionloss += (
